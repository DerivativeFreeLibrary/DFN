...
!
!    Input/output, integer ( kind = 4 ) SEED, the "seed" for the sequence.
...
!
!    Output, real ( kind = 4 ) QUASI(DIM_NUM), the next quasirandom vector.
!
  implicit none

  integer ( kind = 4 ) dim_num
  integer ( kind = 4 ), parameter :: dim_max = 1111
  integer ( kind = 4 ), parameter :: log_max = 30

  integer ( kind = 4 ) atmost
  ...
  integer ( kind = 4 ) k
  integer ( kind = 4 ) l
  integer ( kind = 4 ), save, dimension(dim_max) :: lastq
  integer ( kind = 4 ) m
  integer ( kind = 4 ), save :: maxcol
  integer ( kind = 4 ) newv
  integer ( kind = 4 ), save, dimension(1:dim_max) :: poly
  real ( kind = 4 ) quasi(dim_num)
  real ( kind = 4 ), save :: recipd
  integer ( kind = 4 ) seed
  integer ( kind = 4 ), save :: seed_save = - 1
  integer ( kind = 4 ) seed_temp
  integer ( kind = 4 ), save, dimension(1:dim_max,1:log_max) :: v
...
!
    recipd = real ( l, kind = 4 )
    recipd = 0.5E+00 / recipd
...
!
  quasi(1:dim_num) = real ( lastq(1:dim_num), kind = 4 ) * recipd
  lastq(1:dim_num) = ieor ( lastq(1:dim_num), v(1:dim_num,l) )
