...
subroutine sd_box(n,x,f,bl,bu,alfa_stop,nf_max,ni,nf,iprint,istop, hschoice)
	...
	real*8 :: diff_dense, alfa_d_old, alfa_dense
	real*8 :: alfa_d(n),alfa_diag(n),alfa_coord(n), alfa,alfa_max
	real*8 :: f,fz , eta, fob
...
100	format(' ni=',i4,'  nf=',i5,'   f=',d12.5,'   alfamax=',d12.5)
		...
		if(i_corr.lt.n) then
			...
			if( (dot_product(flag_fail,flag_fail)==n).and.(n>1) ) then
				call gen_base(n,d_dense,direzioni)
				call gram_schmidt(n,direzioni)
...
subroutine funct(n,x,f)
	...
	implicit none
	integer n
	real*8  x(n),f,constr(m), fob, fmax !, viol
