...
!============================================================================================
subroutine sd_box(n,x,f,bl,bu,alfa_stop,nf_max,ni,nf,iprint,istop, hschoice)
	use vincoli
	implicit none

	logical :: cambio_eps
	integer*8 ::n8
	integer :: n,i,j,ii,i_corr,nf,ni,nf_max
	...
	integer :: tipo_direzione, hschoice

	real*8 :: direzioni(n,n)
	real*8 :: dconv(n),dnr
	real*8 :: x(n),z(n),d(n), d_dense(n), d_diag(n)
	real*8 :: viol_old, constr_old(m),d_dense_old(n)
	...
	do i=1,n
		...
		alfa_dense = alfa_dense + alfa_d(i)
		if(iprint.ge.1) then
		      write(*,*) ' alfainiz(',i,')=',alfa_d(i)
		      write(1,*) ' alfainiz(',i,')=',alfa_d(i)
		endif
	end do
	alfa_dense = alfa_dense/dble(n)
	...
	do 
		if(n>1) then
			alfa_max = max(maxval(alfa_coord),maxval(alfa_diag),maxval(alfa_dense))
		else
			alfa_max = maxval(alfa_d)
		endif
...
100        format(' ni=',i4,'  nf=',i5,'   f=',d12.5,'   alfamax=',d12.5)
		...
		if(tipo_direzione == 0) then
			...
			call linesearchbox_cont(n,x,f,d,alfa,alfa_d,z1,fz1,z2,fz2,z,fz,i_corr,num_fal,&
			...
			call linesearchbox_dense(n,x,f,d,alfa,alfa_d(i_corr),z1,fz1,z2,fz2,z,fz,num_fal,&
					   alfa_max,i_corr_fall,iprint,bl,bu,ni,nf)
		...
		if(i_corr.lt.n) then
			...
			if( (maxval(alfa_d).le.soglia).and.(n>1) ) then
				...
				case (0)
					...
					if (iprint .ge.1) write(*,*) 'FINE DIR. COORDINATE'
				...
				case (1)
					...
					if(iprint.ge.1) write(*,*) 'FINE DIR. N+1'
				...
				case (2)
					...
					if(iprint.ge.1) write(*,*) 'FINE DIR. DENSA'
...
subroutine linesearchbox_cont(n,x,f,d,alfa,alfa_d,z1,fz1,z2,fz2,z,fz,i_corr,num_fal,&
                         alfa_max,i_corr_fall,iprint,bl,bu,ni,nf)
	use vincoli
	implicit none
...
subroutine linesearchbox_dense(n,x,f,d,alfa,alfa_d,z1,fz1,z2,fz2,z,fz,num_fal,&
                         alfa_max,i_corr_fall,iprint,bl,bu,ni,nf)
	use vincoli
	implicit none
	...
	integer :: iprint,i_corr_fall
	integer :: ifront,ielle
	real*8 :: x(n),d(n),z(n),bl(n),bu(n),z1(n),z2(n),fz1,fz2
...
subroutine funct(n,x,f)
	use vincoli, only: m,eps,viol
	implicit none
	integer n
	real*8  x(n),f,constr(m), fob, fmax

	call funct_vinc(n, m, x, fob, constr)

	if(m >= 1) then
		fmax = max(0.d0,maxval(constr/eps))
		viol = max(0.d0,maxval(constr))
	else
		fmax = 0.d0
		viol = 0.d0
